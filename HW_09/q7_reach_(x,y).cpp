...
bool isOnOutsideEdge(int n, int m, int x, int y)
{
...
bool bfs(int *arr[], int n, int m, int x, int y, bool *visited[])
{
    static int horizontal[] = {1, 1, 0, 0, -1, -1, 1, -1};
    static int vertical[] = {1, -1, 1, -1, -1, 1, 0, 0};
    // cout << x << ',' << y << " ";
    visited[x][y] = 1;
    Queue<int *> q;
    int temp[] = {x, y};
    ...
    while (!q.empty())
    {
        ...
        {
            int a = u + vertical[i];
            int b = v + horizontal[i];
            if (isSafe(arr, n, m, a, b, visited))
            {
                if (isOnOutsideEdge(n, m, a, b))
                {
                    return 1;
                }
